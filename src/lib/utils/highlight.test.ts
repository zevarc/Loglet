...
describe("compileSearchRegex", () => {
  it("returns null for empty query", () => {
    expect(
      compileSearchRegex("", { regex: false, caseSensitive: false }),
    ).toBeNull();
  });

  it("escapes metachars in substring mode", () => {
    const re = compileSearchRegex("a.b", {
      regex: false,
      caseSensitive: false,
    })!;
  ...
  it("case-sensitive flag respected", () => {
    const re = compileSearchRegex("foo", {
      regex: false,
      caseSensitive: true,
    })!;
  ...
  it("returns null on invalid regex", () => {
    expect(
      compileSearchRegex("[abc", { regex: true, caseSensitive: false }),
    ).toBeNull();
  });
});
